...
if __name__ == "__main__":
    ...
    ff_path = glob(os.path.join(cwd, "*.offxml"))[0]
    pol_handler = "vdW"
    ...
    if pol_handler == "MPIDPolarizability":
        from mpid_plugin.nonbonded import (MPIDCollection, MPIDMultipoleHandler,
    ...
    for idx, mol in enumerate(molecules):
        ...
        if os.path.exists(os.path.join(mol, "conf00")):
        ...
        for conf in confs:
            ...
            data_dict[this_mol.mapped_smiles].append(
                {"respp": respp.tolist(), "respdpol": respdpol.tolist(), "rrms": rrms}
            )
